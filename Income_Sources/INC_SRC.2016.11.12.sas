...
DATA TAX.SCF13_NW;
	RETAIN YY1 NETWORTH ASSET DEBT;
	SET SCF13.SCFP2013;
	WAVE = INPUT(SUBSTR(PUT(Y1,5.),5,1),BEST8.);
	IF WAVE = 1;
	ASSET=1+MAX(LIQ,0)+MAX(CDS,0)+MAX(NMMF,0)+MAX(STOCKS,0)+MAX(BOND,0)+MAX(RETQLIQ,0)+MAX(SAVBND,0)+MAX(CASHLI,0)+MAX(OTHMA,0)+MAX(OTHFIN,0)+MAX(VEHIC,0)+MAX(HOUSES,0)+MAX(ORESRE,0)+MAX(NNRESRE,0)+MAX(BUS,0)+MAX(OTHNFIN,0);
	DROP WAVE;
